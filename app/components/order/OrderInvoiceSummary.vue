...
<template>
  <section class="order-3 bg-white rounded-xl p-5 md:p-6 shadow col-span-8 flex flex-col gap-5">
    ...
    <!-- Service fee -->
    <div v-if="Number(order.visit_cost) > 0" class="flex flex-col gap-1">
      <div class="flex flex-wrap items-center gap-2 justify-between">
      ...
      <p
        v-if="order.visit_fee_credit_info?.eligible && !order.visit_fee_credit_info?.credited"
        v-text="
          $t('order.summary.visit_fee_credit_note', {
            amount: order.visit_fee_credit_info.credit_amount,
            min: order.visit_fee_credit_info.extras_min_price,
