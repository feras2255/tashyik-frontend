...
<template>
  <footer class="mt-auto text-gray-500 bg-[#FCFAFF] border-t border-brand-100/50 pt-16 pb-8">
    <div class="container px-4">
      <!-- 5 Columns Grid (matching original structure) -->
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-10 xl:gap-8 mb-16">
        ...
        <div class="flex flex-col gap-5 items-start text-start">
          ...
          <div class="flex items-center gap-3">
            ...
            <a v-if="social_links?.twitter" :href="social_links?.twitter" aria-label="Twitter" target="_blank" rel="nofollow" class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl hover:bg-brand-800 transition-colors">
              <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 16 16">
            ...
            <a v-if="social_links?.instagram" :href="social_links?.instagram" aria-label="Instagram" target="_blank" rel="nofollow" class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl hover:bg-brand-800 transition-colors">
              <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
            ...
            <a v-if="social_links?.snapchat" :href="social_links?.snapchat" aria-label="Snapchat" target="_blank" rel="nofollow" class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl hover:bg-brand-800 transition-colors">
              <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
            ...
            <a v-if="social_links?.tiktok" :href="social_links?.tiktok" aria-label="TikTok" target="_blank" rel="nofollow" class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl hover:bg-brand-800 transition-colors">
              <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 640 640">
        ...
        <!-- Column 2: Popular Services (from API hub links) -->
        <div class="flex flex-col gap-4">
          <h3 v-text="$t('footer.popular_services')" class="text-gray-900 font-bold text-lg"></h3>
          <div class="flex flex-col gap-2 text-sm font-medium">
            <NuxtLinkLocale
              v-for="service in hubLinks.services"
              :key="service.id"
              :to="{ name: 'services-service', params: { service: resolveEntitySlug(service) } }"
              class="hover:text-brand-700 transition-colors line-clamp-1"
            >
              {{ service.name }}
            </NuxtLinkLocale>
            <NuxtLinkLocale :to="{ name: 'services' }" class="text-brand-700 hover:text-brand-800 font-medium">
              {{ $t('footer.see_all_services') }} →
            </NuxtLinkLocale>
          </div>

          <h3 v-text="$t('footer.popular_cities')" class="text-gray-900 font-bold text-lg mt-4"></h3>
          <div class="flex flex-col gap-2 text-sm font-medium">
            <NuxtLinkLocale
              v-for="city in hubLinks.cities"
              :key="city.id"
              :to="{ name: 'cities-city', params: { city: resolveEntitySlug(city) } }"
              class="hover:text-brand-700 transition-colors line-clamp-1"
            >
              {{ city.name }}
            </NuxtLinkLocale>
            <NuxtLinkLocale :to="{ name: 'cities' }" class="text-brand-700 hover:text-brand-800 font-medium">
        ...
        <div class="flex flex-col gap-4">
          <h3 v-text="$t('footer.download_app')" class="text-gray-900 font-bold text-lg"></h3>
          <div class="flex flex-col gap-3 w-full max-w-[14rem]">
            <a v-if="mobile_app_links?.google_play" :href="mobile_app_links?.google_play" class="bg-white border border-gray-200 shadow-sm rounded-2xl py-2 px-4 flex items-center justify-center gap-3 hover:bg-gray-50 transition-colors w-full">
              <svg class="w-7 h-7" xmlns="http://www.w3.org/2000/svg" width="28.95" height="32" viewBox="0 0 256 283">
              ...
              <div class="flex flex-col items-start leading-tight">
                <span v-text="$t('download_app.get_it_on')" class="text-[10px] text-gray-500 font-medium"></span>
                <span class="text-[14px] font-bold text-gray-900">Google Play</span>
              </div>
            </a>
            <a v-if="mobile_app_links?.app_store" :href="mobile_app_links?.app_store" class="bg-white border border-gray-200 shadow-sm rounded-2xl py-2 px-4 flex items-center justify-center gap-3 hover:bg-gray-50 transition-colors w-full">
              <svg class="w-8 h-8 text-black" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 15 15">
                ...
                <path fill="currentColor" d="M13.35 10.668c.145.106.192.3.11.458C12.14 13.712 11.087 15 10.125 15c-.448 0-.888-.142-1.317-.418a1.99 1.99 0 0 0-2.073-.044c-.52.305-1.015.462-1.485.462c-1.415 0-3.75-4.267-3.75-6.608c0-2.5 1.339-4.406 3.375-4.406q1.437-.001 2.48.419c.294.118.627.11.914-.025c.564-.266 1.308-.394 2.23-.394c1.127 0 2.11.55 2.926 1.615a.36.36 0 0 1-.075.514c-.911.67-1.35 1.421-1.35 2.277c0 .855.439 1.607 1.35 2.276" />
              </svg>
              <div class="flex flex-col items-start leading-tight">
                <span v-text="$t('download_app.download_on_the')" class="text-[10px] text-gray-500 font-medium"></span>
                <span class="text-[14px] font-bold text-gray-900">App Store</span>
              </div>
            </a>
          </div>
        </div>
        ...
        <!-- Column 5: Contact Us -->
        <div class="flex flex-col gap-4">
          <h3 v-text="$t('footer.contact_us.title')" class="text-gray-900 font-bold text-lg"></h3>
          <div v-if="contact_info?.phone_number" class="inline-flex items-center gap-2 text-sm font-medium">
            <div class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl">
              <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
            ...
            <span v-text="contact_info?.phone_number" dir="ltr"></span>
          </div>
          <div v-if="contact_info?.email" class="inline-flex items-center gap-2 text-sm font-medium">
            <div class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl">
              <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
            ...
            <span v-text="contact_info?.email"></span>
          </div>
          <div class="inline-flex items-center gap-2 text-sm font-medium">
            ...
            <span v-text="$t('footer.contact_us.support')"></span>
          </div>
        </div>
      </div>

      <!-- Bottom Bar: Saudi Business Center + Copyright -->
      <div class="border-t border-brand-100/50 pt-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-gray-500 font-medium">
        <!-- Saudi Business Center -->
        <a :href="`https://eauthenticate.saudibusiness.gov.sa/certificate-details/${config.public.saudiBusinessCenterCertificate}`" target="_blank" class="flex items-center gap-3 text-sm">
          <img class="h-11" src="/svg/saudi-business-center.svg" alt="saudi-business-center" />
          <span v-text="$t('footer.saudi_business_center')"></span>
        </a>

        <div class="flex flex-col md:flex-row items-center gap-x-2 gap-y-1 text-center">
          {{ $t('footer.copyright') }} &copy; {{ currentYear }} {{ $t('common.brand') }}.
          <div>
            {{ $t('footer.developed_by') }}
            <a href="https://www.trafyekbls.com/ar" target="_blank" rel="nofollow" class="text-brand-700 underline hover:text-brand-800 font-medium">
              {{ $t('footer.developer') }}
            </a>
          </div>
        </div>
