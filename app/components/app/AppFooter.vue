<script setup>
  ...
  const { data: footerLinksPayload } = useHomeSection('footer_links', async () => {
    try {
  ...
  const hubLinks = computed(() => {
    const raw = footerLinksPayload.value;
    if (raw && (raw.services || raw.cities)) {
      return {
    ...
    <div class="container px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-10 xl:gap-8 mb-16">
        ...
        <div class="flex flex-col gap-5 items-start text-start">
          <AppLogo class="h-12" />
          <p v-if="description" class="text-sm leading-relaxed text-gray-500 font-medium" v-text="description" />
          ...
          <div class="flex items-center gap-3">
            <a
              ...
              target="_blank"
              rel="nofollow"
              class="flex items-center justify-center w-10 h-10 bg-brand-700 text-white rounded-xl hover:bg-brand-800 transition-colors"
            >
        ...
        <div class="flex flex-col gap-4">
          <h3 class="text-gray-900 font-bold text-lg" v-text="$t('footer.popular_services')" />
          ...
          <h3 class="text-gray-900 font-bold text-lg mt-4" v-text="$t('footer.popular_cities')" />
          <div class="flex flex-col gap-2 text-sm font-medium">
            <NuxtLinkLocale
        ...
        <div class="flex flex-col gap-4">
          ...
          <ul class="flex flex-col gap-3 text-sm font-medium">
            <li>
              <NuxtLinkLocale :to="{ name: 'privacy-policy' }" class="hover:text-brand-700 transition-colors">{{
                $t('footer.privacy_policy')
              }}</NuxtLinkLocale>
            </li>
            <li>
              <NuxtLinkLocale :to="{ name: 'terms-and-conditions' }" class="hover:text-brand-700 transition-colors">{{
                $t('footer.terms_and_conditions')
              }}</NuxtLinkLocale>
            </li>
            <li>
              <NuxtLinkLocale to="#" class="hover:text-brand-700 transition-colors">{{ $t('footer.faq') }}</NuxtLinkLocale>
            </li>
            <li>
              <NuxtLinkLocale to="#" class="hover:text-brand-700 transition-colors">{{ $t('footer.contact_us_link') }}</NuxtLinkLocale>
            </li>
          </ul>
        </div>
        ...
        <div class="flex flex-col gap-4">
          <h3 class="text-gray-900 font-bold text-lg" v-text="$t('footer.download_app')" />
          <div class="flex flex-col gap-3 w-full max-w-[14rem]">
            <a
              v-if="mobile_app_links?.google_play"
              :href="mobile_app_links?.google_play"
              class="bg-white border border-gray-200 shadow-sm rounded-2xl py-2 px-4 flex items-center justify-center gap-3 hover:bg-gray-50 transition-colors w-full"
            >
              ...
              <div class="flex flex-col items-start leading-tight">
                <span class="text-[10px] text-gray-500 font-medium" v-text="$t('download_app.get_it_on')" />
                <span class="text-[14px] font-bold text-gray-900">Google Play</span>
              </div>
            </a>
            <a
              v-if="mobile_app_links?.app_store"
            ...
            >
              ...
              <div class="flex flex-col items-start leading-tight">
                <span class="text-[10px] text-gray-500 font-medium" v-text="$t('download_app.download_on_the')" />
        ...
        <!-- Column 5: Contact Us -->
        <div class="flex flex-col gap-4">
          <h3 class="text-gray-900 font-bold text-lg" v-text="$t('footer.contact_us.title')" />
          <div v-if="contact_info?.phone_number" class="inline-flex items-center gap-2 text-sm font-medium">
            ...
            <span dir="ltr" v-text="contact_info?.phone_number" />
          </div>
          <div v-if="contact_info?.email" class="inline-flex items-center gap-2 text-sm font-medium">
            ...
            <span v-text="contact_info?.email" />
          </div>
          <div class="inline-flex items-center gap-2 text-sm font-medium">
            ...
            <span v-text="$t('footer.contact_us.support')" />
          </div>
        </div>
      </div>
      ...
      <div
        class="border-t border-brand-100/50 pt-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-gray-500 font-medium"
      >
        <!-- Saudi Business Center -->
        <a
          :href="`https://eauthenticate.saudibusiness.gov.sa/certificate-details/${config.public.saudiBusinessCenterCertificate}`"
          target="_blank"
          class="flex items-center gap-3 text-sm"
        >
          <img class="h-11" src="/svg/saudi-business-center.svg" alt="saudi-business-center" />
          <span v-text="$t('footer.saudi_business_center')" />
        </a>
        ...
        <div class="flex flex-col md:flex-row items-center gap-x-2 gap-y-1 text-center">
          ...
          <div>
            ...
            <a
              ...
              target="_blank"
              rel="nofollow"
              class="text-brand-700 underline hover:text-brand-800 font-medium"
            >
              {{ $t('footer.developer') }}
            </a>
          </div>
